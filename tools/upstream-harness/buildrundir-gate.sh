#!/usr/bin/env bash
# Sprint: #150; Story: S150.4; Story-ID: c2950820e138
#
# Authenticated 150.4 `buildrundir` packet gate. The upstream Go 1.27 testdir runner
# owns recipe selection, flags, runenv (including GOEXPERIMENT), and terminal
# verdicts; this gate only authenticates inputs, replays the 4 packet roots
...
matrix="$root/docs/upstream-harness/buildrundir-matrix.tsv"
...
check_pin buildrundir_matrix "$matrix"

# Authenticate the exact packet-150.4 root selection: 4 roots whose
# `testdir:<root>` list reproduces the published manifest root-list digest.
tab=$(printf '\t')
rootlist=$(awk -F '\t' '$1 !~ /^#/ && NF == 5 { printf "testdir:%s\n", $2 }' "$matrix")
root_count=$(printf '%s\n' "$rootlist" | wc -l | tr -d ' ')
test "$root_count" = 4 || { printf 'FAIL packet root count is %s, want 4
' "$root_count" >&2; exit 1; }
rootlist_sha=$(printf '%s\n' "$rootlist" | sha256_stdin)
test "$rootlist_sha" = "$(pin_value buildrundir_rootlist)" || {
	printf 'FAIL packet root list digest: expected %s, got %s\n' "$(pin_value buildrundir_rootlist)" "$rootlist_sha" >&2
	exit 1
}
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/bashpp-s1504.XXXXXX")
cleanup() {
...
if test "$seam_fail" -ne 0; then
	printf 'FAIL S150.4 buildrundir packet: backend-seam defect\n' >&2
	exit 1
fi
if test "$product_fail" -ne 0; then
	printf 'NON-GREEN S150.4 buildrundir packet: every root stopped where upstream stopped (the .s generate phase has no Go-source meaning); honest Bash++ product failures are retained for the product-fix sprints\n'
	exit 3
fi
printf 'PASS S150.4 buildrundir packet: 4 authenticated roots through the exact upstream buildrundir action in both modes\n'
