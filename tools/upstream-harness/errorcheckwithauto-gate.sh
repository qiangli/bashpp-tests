#!/usr/bin/env bash
# Sprint: #149; Story: S149.9; Story-ID: 4716cabd8357
#
# Authenticated 149.9 `errorcheckwithauto` packet gate. The upstream Go 1.27 testdir runner
# owns recipe selection, flags, runenv (including GOEXPERIMENT), and terminal
# verdicts; this gate only authenticates inputs, replays the 3 packet roots
...
matrix="$root/docs/upstream-harness/errorcheckwithauto-matrix.tsv"
...
check_pin errorcheckwithauto_matrix "$matrix"

# Authenticate the exact packet-149.9 root selection: 3 roots whose
# `testdir:<root>` list reproduces the published manifest root-list digest.
tab=$(printf '\t')
rootlist=$(awk -F '\t' '$1 !~ /^#/ && NF == 5 { printf "testdir:%s\n", $2 }' "$matrix")
root_count=$(printf '%s\n' "$rootlist" | wc -l | tr -d ' ')
test "$root_count" = 3 || { printf 'FAIL packet root count is %s, want 3\n' "$root_count" >&2; exit 1; }
rootlist_sha=$(printf '%s\n' "$rootlist" | sha256_stdin)
test "$rootlist_sha" = "$(pin_value errorcheckwithauto_rootlist)" || {
	printf 'FAIL packet root list digest: expected %s, got %s\n' "$(pin_value errorcheckwithauto_rootlist)" "$rootlist_sha" >&2
	exit 1
}
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/bashpp-s1499.XXXXXX")
cleanup() {
...
if test "$seam_fail" -ne 0; then
	printf 'FAIL S149.9 errorcheckwithauto packet: backend-seam defect\n' >&2
	exit 1
fi
if test "$product_fail" -ne 0; then
	printf 'NON-GREEN S149.9 errorcheckwithauto packet: every root kept the upstream errorcheckwithauto contract; honest Bash++ product failures are retained for the product-fix sprints\n'
	exit 3
fi
printf 'PASS S149.9 errorcheckwithauto packet: 3 authenticated roots through the exact upstream errorcheckwithauto action\n'
