#!/usr/bin/env bash
# Sprint: #150; Story: S150.7; Story-ID: 803bd644bc67
#
# Authenticated 150.7 `runoutput` packet gate. The upstream Go 1.27 testdir runner
# owns recipe selection, flags, runenv (including GOEXPERIMENT), and terminal
# verdicts; this gate only authenticates inputs, replays the one packet root
...
matrix="$root/docs/upstream-harness/runoutput-matrix.tsv"
...
check_pin runoutput_matrix "$matrix"

# Authenticate the exact packet-150.7 root selection: one root whose
# `testdir:<root>` list reproduces the published manifest root-list digest.
tab=$(printf '\t')
rootlist=$(awk -F '\t' '$1 !~ /^#/ && NF == 5 { printf "testdir:%s\n", $2 }' "$matrix")
root_count=$(printf '%s\n' "$rootlist" | wc -l | tr -d ' ')
test "$root_count" = 1 || { printf 'FAIL packet root count is %s, want 1
' "$root_count" >&2; exit 1; }
rootlist_sha=$(printf '%s\n' "$rootlist" | sha256_stdin)
test "$rootlist_sha" = "$(pin_value runoutput_rootlist)" || {
	printf 'FAIL packet root list digest: expected %s, got %s\n' "$(pin_value runoutput_rootlist)" "$rootlist_sha" >&2
	exit 1
}
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/bashpp-s1507.XXXXXX")
cleanup() {
...
if test "$seam_fail" -ne 0; then
	printf 'FAIL S150.7 runoutput packet: backend-seam defect\n' >&2
	exit 1
fi
if test "$product_fail" -ne 0; then
	printf 'NON-GREEN S150.7 runoutput packet: the root kept the upstream generate-then-execute contract through direct Bash++ runs; honest Bash++ product failures are retained for the product-fix sprints\n'
	exit 3
fi
printf 'PASS S150.7 runoutput packet: one authenticated root through the exact upstream runoutput generator and execute phases in both modes\n'
