#!/usr/bin/env bash
# Sprint: #149; Story: S149.6; Story-ID: 158811e6b5b6
#
# Authenticated 149.6 `build` packet gate. The upstream Go 1.27 testdir runner
# owns recipe selection, flags, runenv (including GOEXPERIMENT), and terminal
# verdicts; this gate only authenticates inputs, replays the four packet roots
...
matrix="$root/docs/upstream-harness/build-matrix.tsv"
...
check_pin build_matrix "$matrix"

# Authenticate the exact packet-149.6 root selection: four roots whose
...
test "$rootlist_sha" = "$(pin_value build_rootlist)" || {
	printf 'FAIL packet root list digest: expected %s, got %s\n' "$(pin_value build_rootlist)" "$rootlist_sha" >&2
	exit 1
}
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/bashpp-s1496.XXXXXX")
cleanup() {
...
if test "$seam_fail" -ne 0; then
	printf 'FAIL S149.6 build packet: backend-seam defect\n' >&2
	exit 1
fi
if test "$product_fail" -ne 0; then
	printf 'NON-GREEN S149.6 build packet: all four roots kept the upstream compile/build-only contract; honest Bash++ product failures are retained for the product-fix sprints\n'
	exit 3
fi
printf 'PASS S149.6 build packet: four authenticated roots through the exact upstream build action in both modes\n'
