#!/usr/bin/env bash
# Sprint: #150; Story: S150.1; Story-ID: a136a527c0b3
#
# Authenticated 150.1 `rundir` packet gate. The upstream Go 1.27 testdir runner
# owns recipe selection, flags, runenv (including GOEXPERIMENT), and terminal
# verdicts; this gate only authenticates inputs, replays the 116 packet roots
...
matrix="$root/docs/upstream-harness/rundir-matrix.tsv"
...
check_pin rundir_matrix "$matrix"

# Authenticate the exact packet-150.1 root selection: 116 roots whose
# `testdir:<root>` list reproduces the published manifest root-list digest.
tab=$(printf '\t')
rootlist=$(awk -F '\t' '$1 !~ /^#/ && NF == 5 { printf "testdir:%s\n", $2 }' "$matrix")
root_count=$(printf '%s\n' "$rootlist" | wc -l | tr -d ' ')
test "$root_count" = 116 || { printf 'FAIL packet root count is %s, want 116\n' "$root_count" >&2; exit 1; }
rootlist_sha=$(printf '%s\n' "$rootlist" | sha256_stdin)
test "$rootlist_sha" = "$(pin_value rundir_rootlist)" || {
	printf 'FAIL packet root list digest: expected %s, got %s\n' "$(pin_value rundir_rootlist)" "$rootlist_sha" >&2
	exit 1
}
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/bashpp-s1501.XXXXXX")
cleanup() {
...
if test "$seam_fail" -ne 0; then
	printf 'FAIL S150.1 rundir packet: backend-seam defect\n' >&2
	exit 1
fi
if test "$product_fail" -ne 0; then
	printf 'NON-GREEN S150.1 rundir packet: every root executed each upstream-selected phase through the seam; honest Bash++ product failures are retained for the product-fix sprints\n'
	exit 3
fi
printf 'PASS S150.1 rundir packet: 116 authenticated roots through the exact upstream rundir phases in both modes\n'
