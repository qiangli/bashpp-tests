...
#
# Authenticated 150.5 `buildrun` packet gate. The upstream Go 1.27 testdir runner
...
matrix="$root/docs/upstream-harness/buildrun-matrix.tsv"
...
check_pin buildrun_matrix "$matrix"

# Authenticate the exact packet-150.5 root selection: one root whose
...
test "$rootlist_sha" = "$(pin_value buildrun_rootlist)" || {
	printf 'FAIL packet root list digest: expected %s, got %s\n' "$(pin_value buildrun_rootlist)" "$rootlist_sha" >&2
	exit 1
}
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/bashpp-s1505.XXXXXX")
cleanup() {
...
if test "$seam_fail" -ne 0; then
	printf 'FAIL S150.5 buildrun packet: backend-seam defect\n' >&2
	exit 1
fi
if test "$product_fail" -ne 0; then
	printf 'NON-GREEN S150.5 buildrun packet: the root kept the upstream build-then-execute contract; honest Bash++ product failures are retained for the product-fix sprints\n'
	exit 3
fi
printf 'PASS S150.5 buildrun packet: one authenticated root through the exact upstream buildrun build and execute phases in both modes\n'
