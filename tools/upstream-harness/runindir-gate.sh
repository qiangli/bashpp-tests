#!/usr/bin/env bash
# Sprint: #150; Story: S150.2; Story-ID: 8f758b9dcd5a
#
# Authenticated 150.2 `runindir` packet gate. The upstream Go 1.27 testdir runner
# owns recipe selection, flags, runenv (including GOEXPERIMENT), and terminal
# verdicts; this gate only authenticates inputs, replays the 10 packet roots
...
matrix="$root/docs/upstream-harness/runindir-matrix.tsv"
...
check_pin runindir_matrix "$matrix"

# Authenticate the exact packet-150.2 root selection: 10 roots whose
# `testdir:<root>` list reproduces the published manifest root-list digest.
tab=$(printf '\t')
rootlist=$(awk -F '\t' '$1 !~ /^#/ && NF == 5 { printf "testdir:%s\n", $2 }' "$matrix")
root_count=$(printf '%s\n' "$rootlist" | wc -l | tr -d ' ')
test "$root_count" = 10 || { printf 'FAIL packet root count is %s, want 10\n' "$root_count" >&2; exit 1; }
rootlist_sha=$(printf '%s\n' "$rootlist" | sha256_stdin)
test "$rootlist_sha" = "$(pin_value runindir_rootlist)" || {
	printf 'FAIL packet root list digest: expected %s, got %s\n' "$(pin_value runindir_rootlist)" "$rootlist_sha" >&2
	exit 1
}
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/bashpp-s1502.XXXXXX")
cleanup() {
...
if test "$seam_fail" -ne 0; then
	printf 'FAIL S150.2 runindir packet: backend-seam defect\n' >&2
	exit 1
fi
if test "$product_fail" -ne 0; then
	printf 'NON-GREEN S150.2 runindir packet: each module program was resolved by the pinned go command and run through the seam; honest Bash++ product failures are retained for the product-fix sprints\n'
	exit 3
fi
printf 'PASS S150.2 runindir packet: 10 authenticated roots through the exact upstream runindir execute phase in both modes\n'
