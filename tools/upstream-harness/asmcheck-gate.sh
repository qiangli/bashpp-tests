#!/usr/bin/env bash
# Sprint: #149; Story: S149.3; Story-ID: 60d35d1ec914
#
# Authenticated 149.3 `asmcheck` packet gate. The upstream Go 1.27 testdir runner
# owns recipe selection, flags, runenv (including GOEXPERIMENT), and terminal
# verdicts; this gate only authenticates inputs, replays the 84 packet roots
...
matrix="$root/docs/upstream-harness/asmcheck-matrix.tsv"
...
check_pin asmcheck_matrix "$matrix"

# Authenticate the exact packet-149.3 root selection: 84 roots whose
# `testdir:<root>` list reproduces the published manifest root-list digest.
tab=$(printf '\t')
rootlist=$(awk -F '\t' '$1 !~ /^#/ && NF == 5 { printf "testdir:%s\n", $2 }' "$matrix")
root_count=$(printf '%s\n' "$rootlist" | wc -l | tr -d ' ')
test "$root_count" = 84 || { printf 'FAIL packet root count is %s, want 84\n' "$root_count" >&2; exit 1; }
rootlist_sha=$(printf '%s\n' "$rootlist" | sha256_stdin)
test "$rootlist_sha" = "$(pin_value asmcheck_rootlist)" || {
	printf 'FAIL packet root list digest: expected %s, got %s\n' "$(pin_value asmcheck_rootlist)" "$rootlist_sha" >&2
	exit 1
}
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/bashpp-s1493.XXXXXX")
cleanup() {
...
seam_fail=0
product_fail=0
# asmcheck compares generated assembly, which only compiled mode produces;
# interpreted mode is an explicit unsupported disposition and is not replayed.
for mode in compiled; do
...
if test "$seam_fail" -ne 0; then
	printf 'FAIL S149.3 asmcheck packet: backend-seam defect\n' >&2
	exit 1
fi
if test "$product_fail" -ne 0; then
	printf 'NON-GREEN S149.3 asmcheck packet: every root kept the upstream asmcheck contract; honest Bash++ product failures are retained for the product-fix sprints\n'
	exit 3
fi
printf 'PASS S149.3 asmcheck packet: 84 authenticated roots through the exact upstream asmcheck action\n'
