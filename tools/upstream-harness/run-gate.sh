...
#
# Authenticated 150.6 `run` packet gate. The upstream Go 1.27 testdir runner
# owns recipe selection, flags, runenv (including GOEXPERIMENT), and terminal
# verdicts; this gate only authenticates inputs, replays the 48 packet roots
...
matrix="$root/docs/upstream-harness/run-matrix.tsv"
...
check_pin run_matrix "$matrix"

# Authenticate the exact packet-150.6 root selection: 48 roots whose
# `testdir:<root>` list reproduces the published manifest root-list digest.
tab=$(printf '\t')
rootlist=$(awk -F '\t' '$1 !~ /^#/ && NF == 5 { printf "testdir:%s\n", $2 }' "$matrix")
root_count=$(printf '%s\n' "$rootlist" | wc -l | tr -d ' ')
test "$root_count" = 48 || { printf 'FAIL packet root count is %s, want 48\n' "$root_count" >&2; exit 1; }
rootlist_sha=$(printf '%s\n' "$rootlist" | sha256_stdin)
test "$rootlist_sha" = "$(pin_value run_rootlist)" || {
	printf 'FAIL packet root list digest: expected %s, got %s\n' "$(pin_value run_rootlist)" "$rootlist_sha" >&2
	exit 1
}
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/bashpp-s1506.XXXXXX")
cleanup() {
...
if test "$seam_fail" -ne 0; then
	printf 'FAIL S150.6 run packet: backend-seam defect\n' >&2
	exit 1
fi
if test "$product_fail" -ne 0; then
	printf 'NON-GREEN S150.6 run packet: every root reached the upstream execute phase through a direct Bash++ run; honest Bash++ product failures are retained for the product-fix sprints\n'
	exit 3
fi
printf 'PASS S150.6 run packet: 48 authenticated roots through the exact upstream run action in both modes\n'
