#!/usr/bin/env bash
# Sprint: #149; Story: S149.1; Story-ID: 3f416ade73ef
#
# Authenticated 149.1 `errorcheck` packet gate. The upstream Go 1.27 testdir runner
# owns recipe selection, flags, runenv (including GOEXPERIMENT), and terminal
# verdicts; this gate only authenticates inputs, replays the 144 packet roots
...
matrix="$root/docs/upstream-harness/errorcheck-matrix.tsv"
...
check_pin errorcheck_matrix "$matrix"

# Authenticate the exact packet-149.1 root selection: 144 roots whose
# `testdir:<root>` list reproduces the published manifest root-list digest.
tab=$(printf '\t')
rootlist=$(awk -F '\t' '$1 !~ /^#/ && NF == 5 { printf "testdir:%s\n", $2 }' "$matrix")
root_count=$(printf '%s\n' "$rootlist" | wc -l | tr -d ' ')
test "$root_count" = 144 || { printf 'FAIL packet root count is %s, want 144\n' "$root_count" >&2; exit 1; }
rootlist_sha=$(printf '%s\n' "$rootlist" | sha256_stdin)
test "$rootlist_sha" = "$(pin_value errorcheck_rootlist)" || {
	printf 'FAIL packet root list digest: expected %s, got %s\n' "$(pin_value errorcheck_rootlist)" "$rootlist_sha" >&2
	exit 1
}
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/bashpp-s1491.XXXXXX")
cleanup() {
...
if test "$seam_fail" -ne 0; then
	printf 'FAIL S149.1 errorcheck packet: backend-seam defect\n' >&2
	exit 1
fi
if test "$product_fail" -ne 0; then
	printf 'NON-GREEN S149.1 errorcheck packet: every root kept the upstream errorcheck contract; honest Bash++ product failures are retained for the product-fix sprints\n'
	exit 3
fi
printf 'PASS S149.1 errorcheck packet: 144 authenticated roots through the exact upstream errorcheck action\n'
