#!/usr/bin/env bash
# Sprint: #149; Story: S149.8; Story-ID: 84295fc6a058
#
# Authenticated 149.8 `errorcheckoutput` packet gate. The upstream Go 1.27 testdir runner
...
matrix="$root/docs/upstream-harness/errorcheckoutput-matrix.tsv"
...
check_pin errorcheckoutput_matrix "$matrix"

# Authenticate the exact packet-149.8 root selection: 4 roots whose
...
test "$rootlist_sha" = "$(pin_value errorcheckoutput_rootlist)" || {
	printf 'FAIL packet root list digest: expected %s, got %s\n' "$(pin_value errorcheckoutput_rootlist)" "$rootlist_sha" >&2
	exit 1
}
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/bashpp-s1498.XXXXXX")
cleanup() {
...
if test "$seam_fail" -ne 0; then
	printf 'FAIL S149.8 errorcheckoutput packet: backend-seam defect\n' >&2
	exit 1
fi
if test "$product_fail" -ne 0; then
	printf 'NON-GREEN S149.8 errorcheckoutput packet: every root kept the upstream errorcheckoutput contract; honest Bash++ product failures are retained for the product-fix sprints\n'
	exit 3
fi
printf 'PASS S149.8 errorcheckoutput packet: 4 authenticated roots through the exact upstream errorcheckoutput action\n'
