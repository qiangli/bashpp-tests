#!/usr/bin/env bash
# Sprint: #149; Story: S149.2; Story-ID: 1e87cb008ec3
#
# Authenticated 149.2 `compiledir` packet gate. The upstream Go 1.27 testdir runner
# owns recipe selection, flags, runenv (including GOEXPERIMENT), and terminal
# verdicts; this gate only authenticates inputs, replays the 125 packet roots
...
matrix="$root/docs/upstream-harness/compiledir-matrix.tsv"
...
check_pin compiledir_matrix "$matrix"

# Authenticate the exact packet-149.2 root selection: 125 roots whose
# `testdir:<root>` list reproduces the published manifest root-list digest.
tab=$(printf '\t')
rootlist=$(awk -F '\t' '$1 !~ /^#/ && NF == 5 { printf "testdir:%s\n", $2 }' "$matrix")
root_count=$(printf '%s\n' "$rootlist" | wc -l | tr -d ' ')
test "$root_count" = 125 || { printf 'FAIL packet root count is %s, want 125\n' "$root_count" >&2; exit 1; }
rootlist_sha=$(printf '%s\n' "$rootlist" | sha256_stdin)
test "$rootlist_sha" = "$(pin_value compiledir_rootlist)" || {
	printf 'FAIL packet root list digest: expected %s, got %s\n' "$(pin_value compiledir_rootlist)" "$rootlist_sha" >&2
	exit 1
}
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/bashpp-s1492.XXXXXX")
cleanup() {
...
if test "$seam_fail" -ne 0; then
	printf 'FAIL S149.2 compiledir packet: backend-seam defect\n' >&2
	exit 1
fi
if test "$product_fail" -ne 0; then
	printf 'NON-GREEN S149.2 compiledir packet: every root kept the upstream compiledir contract; honest Bash++ product failures are retained for the product-fix sprints\n'
	exit 3
fi
printf 'PASS S149.2 compiledir packet: 125 authenticated roots through the exact upstream compiledir action\n'
