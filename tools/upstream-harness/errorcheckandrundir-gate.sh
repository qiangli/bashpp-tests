#!/usr/bin/env bash
# Sprint: #150; Story: S150.3; Story-ID: 4d5c5fab536e
#
# Authenticated 150.3 `errorcheckandrundir` packet gate. The upstream Go 1.27 testdir runner
# owns recipe selection, flags, runenv (including GOEXPERIMENT), and terminal
# verdicts; this gate only authenticates inputs, replays the 5 packet roots
...
matrix="$root/docs/upstream-harness/errorcheckandrundir-matrix.tsv"
...
check_pin errorcheckandrundir_matrix "$matrix"

# Authenticate the exact packet-150.3 root selection: 5 roots whose
# `testdir:<root>` list reproduces the published manifest root-list digest.
tab=$(printf '\t')
rootlist=$(awk -F '\t' '$1 !~ /^#/ && NF == 5 { printf "testdir:%s\n", $2 }' "$matrix")
root_count=$(printf '%s\n' "$rootlist" | wc -l | tr -d ' ')
test "$root_count" = 5 || { printf 'FAIL packet root count is %s, want 5\n' "$root_count" >&2; exit 1; }
rootlist_sha=$(printf '%s\n' "$rootlist" | sha256_stdin)
test "$rootlist_sha" = "$(pin_value errorcheckandrundir_rootlist)" || {
	printf 'FAIL packet root list digest: expected %s, got %s\n' "$(pin_value errorcheckandrundir_rootlist)" "$rootlist_sha" >&2
	exit 1
}
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/bashpp-s1503.XXXXXX")
cleanup() {
...
if test "$seam_fail" -ne 0; then
	printf 'FAIL S150.3 errorcheckandrundir packet: backend-seam defect\n' >&2
	exit 1
fi
if test "$product_fail" -ne 0; then
	printf 'NON-GREEN S150.3 errorcheckandrundir packet: every root executed each upstream-selected phase through the seam; honest Bash++ product failures are retained for the product-fix sprints\n'
	exit 3
fi
printf 'PASS S150.3 errorcheckandrundir packet: 5 authenticated roots through the exact upstream errorcheckandrundir phases in both modes\n'
